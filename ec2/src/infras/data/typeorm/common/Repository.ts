...
export abstract class Repository<TEntity extends Entity, TDbEntity extends DbEntity<TEntity>> {
  ...
  async createMultiple(list: TEntity[], querySession?: DbQuerySession): Promise<string[]> {
    ...
    const result = await this.repository.createQueryBuilder(this._schema.TABLE_NAME, querySession).insert().values(arr).execute();
  ...
  async updateFields(id: string, data: TEntity, fields: UpdateFieldQuery<TEntity>[], querySession?: DbQuerySession): Promise<boolean> {
    ...
    const result = await this.repository.createQueryBuilder(this._schema.TABLE_NAME, querySession).update(obj).whereInIds(id).execute();
    return !!result.affected;
  }

  async delete(id: string, querySession?: DbQuerySession): Promise<boolean> {
    const result = await this.repository.createQueryBuilder(this._schema.TABLE_NAME, querySession).delete().whereInIds(id).execute();
    return !!result.affected;
  }

  async deleteMultiple(ids: string[], querySession?: DbQuerySession): Promise<boolean> {
    const result = await this.repository.createQueryBuilder(this._schema.TABLE_NAME, querySession).delete().whereInIds(ids).execute();
    return !!result.affected && result.affected === ids.length;
  }

  async softDelete(id: string, querySession?: DbQuerySession): Promise<boolean> {
    const result = await this.repository.createQueryBuilder(this._schema.TABLE_NAME, querySession).softDelete().whereInIds(id).execute();
    return !!result.affected;
  }

  async softDeleteMultiple(ids: string[], querySession?: DbQuerySession): Promise<boolean> {
    const result = await this.repository.createQueryBuilder(this._schema.TABLE_NAME, querySession).softDelete().whereInIds(ids).execute();
    return !!result.affected && result.affected === ids.length;
  }

  async restore(id: string, querySession?: DbQuerySession): Promise<boolean> {
    const result = await this.repository.createQueryBuilder(this._schema.TABLE_NAME, querySession).restore().whereInIds(id).execute();
    return !!result.affected;
  }

  async restoreMultiple(ids: string[], querySession?: DbQuerySession): Promise<boolean> {
    const result = await this.repository.createQueryBuilder(this._schema.TABLE_NAME, querySession).restore().whereInIds(ids).execute();
    return !!result.affected && result.affected === ids.length;
  }
}
